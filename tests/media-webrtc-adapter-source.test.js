...
test("media webrtc adapter owns runtime media transport profile helpers", () => {
  ...
  assert.match(adapter, /export type SurfaceAdapterBindingPosture/);
  assert.match(adapter, /export type MediaWebRtcAdapterBindingProfile/);
  assert.match(adapter, /export function mediaWebRtcAdapterBindingProfile/);
  ...
  assert.match(adapter, /adapterBindingPosture\?: SurfaceAdapterBindingPosture \| null/);
  assert.match(adapter, /fulfillmentSessionId: string/);
  assert.match(adapter, /fulfillmentSessionId: string/);
  assert.match(adapter, /requires runtime-prepared fulfillmentSessionId/);
  assert.doesNotMatch(adapter, /browserFulfillmentSessionId/);
  assert.match(adapter, /fulfillmentSessionId: session\.fulfillmentSessionId/);
  assert.match(adapter, /fulfillmentSessionId: firstString\(evidence\.fulfillmentSessionId, session\.fulfillmentSessionId\)/);
  assert.match(adapter, /adapterModuleRef: adapterBinding\.adapterModuleRef/);
  assert.match(adapter, /adapterModuleRef: session\.adapterModuleRef \|\| BROWSER_STREAM_ADAPTER_REF/);
  assert.match(adapter, /transportProfileRefs: session\.transportProfileRefs/);
  assert.match(adapter, /renderEvidenceBudgetRef: session\.renderEvidenceBudgetRef/);
  assert.match(adapter, /function trackStateFromEvidence\(/);
  assert.match(adapter, /SWARM\.MEDIA_FULFILLMENT_EVIDENCE_KIND\.TRACK_STATE/);
  assert.match(adapter, /SWARM\.MEDIA_TRANSPORT_TRACK_STATE\.LIVE/);
  assert.match(adapter, /trackState: trackStateFromEvidence\(evidence\)/);
  assert.match(adapter, /function mediaCorrelationKeys\(/);
  assert.match(adapter, /function mediaCorrelationMaterializationBudget\(/);
  assert.match(adapter, /sourceAuthority = BROWSER_STREAM_ADAPTER_REF/);
  ...
  assert.match(adapter, /correlationBudget: mediaCorrelationMaterializationBudget\(correlationKeys, issuedAt, adapterBinding\.adapterModuleRef\)/);
